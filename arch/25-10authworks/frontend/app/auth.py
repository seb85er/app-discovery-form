...
def login_user(username, password):
    """
    Handles the login logic for a user, including retrieving their role from the roles container.
    """
    list_all_users()
    ...
    if user:
        if verify_password(user["password_hash"], password):
            ...
            try:
                ...
                if roles:
                    role = roles[0].get("role", "user")
                    session["user_role"] = role
                    logger.info(f"User '{username}' logged in successfully with role '{role}'.")
                else:
                    session["user_role"] = "user"
                    logger.warning(f"No role found for user '{username}', defaulting to 'user'.")
