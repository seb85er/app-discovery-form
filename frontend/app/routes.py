...
@app.route("/admin/assign-application", methods=["POST"])
@role_required("admin")
def assign_application():
    user_id = request.form.get("user_id")
    app_id = request.form.get("app_id")
    
    user = find_user_by_id(user_id)  # Add or adjust this function to retrieve user by ID
    if user:
        assigned_apps = user.get("assigned_apps", [])
        if app_id not in assigned_apps:
            assigned_apps.append(app_id)
            user["assigned_apps"] = assigned_apps
            users_container.replace_item(item=user_id, body=user)
            flash(f"Application {app_id} assigned to {user['username']}.", "success")
        else:
            flash(f"User already has access to Application {app_id}.", "info")
    else:
